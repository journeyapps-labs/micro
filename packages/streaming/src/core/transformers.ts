...
export type ExtractStreamElementType<T extends StreamLike<any>> =
  T extends Iterable<infer I> ? I : T extends AsyncIterable<infer I> ? I : never;
 ...
 */
export async function* concat<S extends StreamLike<any>>(...sources: S[]): AsyncGenerator<ExtractStreamElementType<S>> {
 ...
 */
export async function* map<I, O>(iterable: StreamLike<I>, transform: (element: I) => O | Promise<O>) {
 ...
 */
export async function* filter<I>(iterable: StreamLike<I>, comparator: (element: I) => boolean | Promise<boolean>) {
  for await (const element of iterable) {
...
const isReducedValue = <T>(value: any | ReducedValue<T>): value is ReducedValue<T> => {
...
export const reduce = async <I, A>(
  ...
  reducer: (accumulator: A, element: I) => A | ReducedValue<A> | Promise<A | ReducedValue<A>>,
  init: A
) => {
