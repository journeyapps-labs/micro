...
describe("bson-transformer", () => {
  test("it successfully reads data from chunks array", () => {
    ...
    expect(
      micro_streaming.bson.readBufferFromChunks(chunks, 5)?.chunks_read,
    ).toBe(4);
    expect(
      micro_streaming.bson.readBufferFromChunks(chunks, 2)?.buffer.length,
    ).toBe(3);
  });
  ...
  test("it successfully serializes objects to bson stream", async () => {
    ...
    const sink = source.pipeThrough(
      micro_streaming.bson.createBSONStreamEncoder(),
    );
