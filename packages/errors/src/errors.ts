...
export type ErrorData = {
  name?: string;
  /**
   * enum such as `VALIDATION_ERROR` etc..
   */
  code: string;
  /**
   * Human-readable description of the error. This is typically shown to the user in various UI
   */
  description: string;
  ...
  trace_id?: string;
  /**
   * If this error should be reported to tools such as Sentry.
   * In some cases we may deliberately throw validation errors to show information to end users which are picked
   * up by our routes, and then we don't want these reported (in which case set this to false)
   *
   * @default true
   */
  report?: boolean;
...
export class JourneyError extends Error {
  ...
  get shouldReport() {
    return this.errorData.report ?? true;
  }
